...
{
    public class ScenarioReqWorkEndParamValues : ScenarioParamValues
    {
        public ScenarioReqWorkEndParamValues(List<string> values, bool useEventHandling, string dataPathToUpload, WaferMapData mapData) : base(values)
        {
    ...
    public class ScenarioReqWorkEnd : ScenarioBaseClass
    {
        #region <Constructors>
        public ScenarioReqWorkEnd(string name, long eventId, List<long> variables,
            long vidPmsBody,
            ...
            : base(name, timeOut)
        {
            ...
            {
                UsePmsHandling = false;
            }

            if (streamToHandling < 0 || funcToSetupDataSend < 0 || funcToDataTransmitInquire < 0 || funcToDataSend < 0)
            {
                UseMapHandling = false;
            }
            else
            {
                UseMapHandling = true;
        ...
        #region <Fields>
        private ScenarioReqWorkEndParamValues _paramValue = null;
        private List<SemiObject> _receiveMessageFormat = null;
        private readonly List<SemiObject> MessageFormatToSend = null;

        private readonly bool UseMapHandling;
        private readonly bool UsePmsHandling;
        ...
        {
            INIT = 0,
            UPDATE_BINARY_VARIABLE = 100,
            SEND_EVENT = 200,
            SEND_SECSMESSAGE_MAP_SETUP_DATA = 300,
            SEND_SECSMESSAGE_MAP_TRANSMIT_INQUIRE = 400,
            SEND_SECSMESSAGE_MAP_DATA = 500,
            FINISH = 1000,
        }
        #endregion </Types>
        ...
        //        if (_paramValue == null)
        //        {
        //            _paramValue = new ScenarioReqWorkEndParamValues(null, new WaferMapData());
        //        }
                ...
                case (int)ScenarioSeq.INIT:
                    {
                        ...
                        if (_paramValue != null)
                        {
                            if (false == _paramValue.UseEventHandling)
                            {
        ...
        {
            _paramValue = paramValues as ScenarioReqWorkEndParamValues;
        }
