...
using EFEM.CustomizedByCustomer.PWA500BIN;
...
namespace FrameOfSystem3.SECSGEM.Scenario
{
    public class ProcessingScenarioPWA500BIN_TP : ProcessingScenario
    {
            ...
            foreach (ScenarioListTypes scenario in Enum.GetValues(typeof(ScenarioListTypes)))
            {
                ...
                switch (scenario)
                {
                    ...
                    case ScenarioListTypes.SCENARIO_REQ_RECIPE_DOWNLOAD:
                        {
                            MakeScenario(scenario, new EFEM.CustomizedByCustomer.PWA500BIN.ScenarioRecipeHandlingRequest(scenario.ToString(),
                                false,
        ...
        public override List<string> GetScenarioParameterList(Enum scenario)
        {
            ...
            else if (scen is EFEM.CustomizedByCustomer.PWA500BIN.ScenarioRecipeHandlingRequest)
            {
                parameterList = new List<string>();
            ...
            else if (scen is ScenarioReqWorkEnd)
            {
                ...
                parameterList.Add(UploadCoreOrBinFileKeys.KeyUseEventHandling);
                //foreach (var item in CollectionEventList)
                //{
                //    if (item.Value.Id.Equals(convertedScen.EventId))
                //    {
                ...
                case ScenarioListTypes.SCENARIO_REQ_RECIPE_DOWNLOAD:
                    {
                        ...
                        ScenarioList[scenario].UpdateParamValues(new EFEM.CustomizedByCustomer.PWA500BIN.RecipeHandlingRequestParamValues(NameOfClient, MessagesToSend.RequestDownloadRecipe.ToString(),
                ...
                case ScenarioListTypes.SCENARIO_REQ_RECIPE_UPLOAD:
                    {
                        ...
                        param.Remove(RecipeHandlingKeys.KeyUseCommunicationToPM);
                        ScenarioList[scenario].UpdateParamValues(new EFEM.CustomizedByCustomer.PWA500BIN.RecipeHandlingRequestParamValues(NameOfClient, MessagesToSend.RequestUploadRecipe.ToString(),
                            paramToSend, useCommunication,
                ...
                case ScenarioListTypes.SCENARIO_BIN_DATA_UPLOAD:
                    {
                        ...
                        ScenarioList[scenario].UpdateParamValues(new ScenarioReqWorkEndParamValues(vids, useEventHandling, param[UploadCoreOrBinFileKeys.KeyPMSFileBody], waferMapData));
                    }
                    break;
        ...
        public override bool RemoteCommandReceived(string rcmdName, string[] cpNames, string[] cpValues, ref long[] results)
        {
            ...
            else
            {
                ...
                foreach (var item in ScenarioList)
                {
                    ...
                    else if (item.Value is EFEM.CustomizedByCustomer.PWA500BIN.ScenarioRecipeHandlingRequest)
                    {
                        var eventScenario = item.Value as EFEM.CustomizedByCustomer.PWA500BIN.ScenarioRecipeHandlingRequest;
                        if (eventScenario.EventId.Equals(ceid))
                        {
                            item.Value.UpdatePermission(permission);
                            return true;
        ...
        private string GetPMSFileName(string lotId, string substrateId)
        {
            return string.Format("{0}_{1}_{2}_{3}", GetModelName(), lotId, substrateId, DateTime.Now.ToString("yyMMddHHmmss"));
        }
        private bool MakePMSFile(string lotId, string substrateId, string fileName, string body, ref string fullPath)
        {
