...
namespace FrameOfSystem3.Task
{
    abstract class TaskLoadPort : RunningTaskWrapper
    {
            ...
            : base(nIndexOfTask, strTaskName, typeof(PARAM_PROCESS))
        {
            ...
            int indexCassetteOutput = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_OUT.LP1_MANUAL_CASSETTE + relIndexOutput;
            int indexFoupOutput = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_OUT.LP1_MANUAL_MAC_FOUP + relIndexOutput;
            ...
            int relIndexInput = LoadPortIndex * 8;
            int indexInputCassetteChanger = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_MANUAL_BUTTON_CASSETTE_STATUS + relIndexInput;
            int indexInputFoupChanger = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_MANUAL_BUTTON_MAC_FOUP_STATUS + relIndexInput;
            ...
            int indexInputCassetteMode = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_PLACEMENT_CASSETTE_STATUS + relIndexInput;
            int indexInputFoupMode = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_PLACEMENT_MAC_FOUP_STATUS + relIndexInput;
            ...
            CarrierPresenceIndex = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_PRESENT_STATUS + relIndexInput;
            BusyStatusIndex = (int)Define.DefineEnumProject.DigitalIO.EN_DIGITAL_IN.LP1_RUN + relIndexInput;
        ...
        protected readonly ConcurrentDictionary<LoadPortMode, bool> TriggerChangingMode;
        ...
        protected readonly int BusyStatusIndex;
        ...
        protected const uint DefualtWaitTime = 180000;
        #endregion </LoadPort>
                ...
                case (int)STEP_INITIALIZE.CHANGE_MODE + 1:
                    {
                        ...
                        result = _loadPortManager.ChangeLoadPortMode(LoadPortIndex, loadingType);
                        switch (result.CommandResult)
                        {
                            case CommandResult.Completed:
                                {
        ...
        protected virtual bool ActionWaitForLoading()
        {
            ...
            switch (m_nSeqNum)
            {
                ...
                case (int)STEP_WAIT_FOR_LOADING.CALL_OHT_CARRIER_TO_LOAD + 1:
                    {
                        ...
                        switch (result.CommandResult)
                        {
                            ...
                            case CommandResult.Skipped:
							    // TODO : Here
                                int waitTime = GetParameter(Recipe.PARAM_EQUIPMENT.HandlingWaitTime, 180000);
                ...
                case (int)STEP_WAIT_FOR_LOADING.SAFTY_INTERLOCK_DETECTED:
                    {
                        // TODO : 정지 시그널용 인터페이스 검증 필요 -> 인덱스
                        int index = LoadPortIndex * 8 + 2;
                        _loadPortManager.WriteAMHSOutput(LoadPortIndex, index, true);
                ...
                case (int)STEP_WAIT_FOR_UNLOADING.CALL_OHT_CARRIER_TO_UNLOAD + 1:
                    {
                        ...
                        switch (result.CommandResult)
                        {
                            case CommandResult.Completed:
                            case CommandResult.Skipped:
                                uint waitTime = (uint)GetParameter(Recipe.PARAM_EQUIPMENT.HandlingWaitTime, DefualtWaitTime);
                                TicksForHandling.SetTickCount(waitTime);
                                m_nSeqNum = (int)STEP_WAIT_FOR_UNLOADING.INTERFACE_BY_PIO;
                                break;
                            case CommandResult.Timeout:
                            case CommandResult.Error:
                            ...
                            case CommandResult.Completed:
                                {
                                    ...
                                    // 올 수가 없다.
                                    if (false == IsAnyCarrierPlaced(ref _loadingMode))
                                    {
                            ...
                            case TASK_ACTION.CHANGE_LOADPORT_LOADING_MODE_TO_CASSETTE:
                                {
                                    ...
                                    LoadPortLoadingMode loadingType = LoadPortLoadingMode.Unknown;
                                    bool hasAnyCarrier = IsAnyCarrierPlaced(ref loadingType);
        ...
        private void ChangingModeButtonClicked(LoadPortLoadingMode loadingType)
        {
            TASK_ACTION action;
            switch (loadingType)
            {
        ...
        protected bool IsLoadPortBusy()
        {
            return DigitalIO_.DigitalIO.GetInstance().ReadInput(BusyStatusIndex);
        }
